...
public class WorldPanel extends JPanel implements MouseListener, ActionListener {
    ...
    public int RI = random.nextInt(1);
    private Timer timer;
...
for(int i = 0; i < cellArray.length; i++) {
    ...
    public void step() {
        ...
        for(int i = 0; i < cellArray.length; i++) {
    		for(int j = 0; j < cellArray[i].length; j++) {
    		//cellArray[i][j].	
        cellArray[i][j].liveOrDie(getLivingNeighbors(null, 0, 0));
    		}
    		}
    ...
    public void mousePressed(MouseEvent e) {
    	...
    	int x = e.getX();
    	int y = e.getY();
    	int cellClickedX = x/cellSize;
    	int cellClickedY = y/cellSize;
    	if(cellArray[x][y].isAlive == true) {
    		cellArray[x][y].isAlive = false;
    	}
    	else {
